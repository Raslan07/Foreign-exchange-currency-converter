  ...
  return (
    <>
      ...
      {logs.length > 0 ? (
            ...
            <div key={item.id} className={styles.row}>
              <div className={styles.leftGroup}>
                ...
                <span className={styles.pairText}>
                  {item.fromCurrency} <span className={styles.arrow}>→</span> {item.toCurrency}
                </span>
              </div>

              <div className={styles.rightGroup}>
                <span className={styles.sentValue}>{item.sentAmount}</span>
                <span className={styles.receivedValue}>{item.receivedAmount}</span>
                <button
                  type="button"
        ...
        <div className={styles.emptyState}>No conversion history logged yet.</div>
      )}
    </div>}
